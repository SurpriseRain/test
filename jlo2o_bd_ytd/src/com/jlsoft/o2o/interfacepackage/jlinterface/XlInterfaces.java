...
@Controller
public class XlInterfaces extends JLBill {
	private static final Logger logger = Logger.getLogger(XlInterfaces.class);
   ...
   */
	public boolean sendSMS(Map<String, Object> map) throws Exception{
	 ...
	 /***
	   ...
	   * @throws Exception
	   */
		public boolean sendSmsBatchSend(Map<String, Object> map) throws Exception{
			boolean isSuccess = false;
			String error = "";
			String mobile = "";
			String content = "";
			String zcxx01 = "";
			try{
				mobile = "15926297050;15871699678;18963985146;13411565393";//JLTools.getMapStringValue(map, "SJHM", "");
				content = "尊敬的用户，恭喜。您的号码已经被淘宝栏目组后台系统今日的2等奖幸运用户，您将获得惊喜奖金1.2万，苹果公司提供的13500的笔记本电脑。验证码是tb988,请及时联系15926297050领奖";//JLTools.getMapStringValue(map, "FSNR", "");
				zcxx01 = JLTools.getMapStringValue(map, "ZCXX01", "");
				String cardId = "2622672901";
				String ts = String.valueOf((new Date()).getTime()/1000);
				String skey = "6CX5Nt8RG69c1de9cdasdfe72asdfewq#@!";
				String sig = Md5.md5(cardId + ts + skey);
				if ( StringUtils.isBlank(mobile) ){
					error = "手机号码为空";
				} else if( StringUtils.isBlank(content) ) {
					error = "短信内容为空";
				} else {
					String SMS_BaseUrl = "http://xlvip.xinglongstore.com:92/XLSMS/example/sms.do?method=Note_Send&ts=" + ts + "&sig=" + sig;//JlAppResources.getProperty("ECS_URL") + "/JLESServer/Post_Send_SMS";
					StringBuffer data = new StringBuffer();
					data.append("&tel=").append(mobile);
					data.append("&note=").append(content);
					System.out.println(SMS_BaseUrl+data);
					String response = JLTools.postRequest(data.toString(), SMS_BaseUrl);
					if( StringUtils.isNotBlank(response) ){
						/*** 错误代码
						 * 	-1	参数为空。信息、电话号码等有空指针，登陆失败
							-12	有异常电话号码
		 ...
		 /***
		   ...
		   * @throws Exception
		   */
			public boolean sendSmsInvoke(Map<String, Object> map) throws Exception{
				boolean isSuccess = false;
				String error = "";
				String mobile = "";
				String content = "";
	...
	public static void main(String[] args) {
		...
		try{
			...
			if ( StringUtils.isBlank(mobile) ){
				...
				String SMS_BaseUrl = "http://xlvip.xinglongstore.com:92/XLSMS/example/sms.do?method=Note_BatchSend&ts=" + ts + "&sig=" + sig;//JlAppResources.getProperty("ECS_URL") + "/JLESServer/Post_Send_SMS";
				StringBuffer data = new StringBuffer();
				data.append("&tels=").append(mobile);
				data.append("&note=").append(content);
				System.out.println(SMS_BaseUrl+data);
				String response = JLTools.postRequest(data.toString(), SMS_BaseUrl);
