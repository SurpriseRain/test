...
<%
	...
	//支付金额，格式：元.角分
	String signType 	= request.getParameter("signType");		//货币代码
	String payAmount 	= request.getParameter("payAmount");	
	//系统参考号
	String paymentOrderId 	= request.getParameter("paymentOrderId");
	String payResult 	= request.getParameter("payResult");//响应码
	//清算日期，格式：月月日日
...
%>
	var JYDH = '<%=OrderNo%>';
	var paymentOrderId = "<%=paymentOrderId%>";
	var PayNo = "<%=paymentOrderId%>";
	var SignMsg = "<%=payDatetime%>";
	var JYJE = "<%=payAmount%>";
	//插入银联记录表
	var jsonData = {"JYDH":JYDH,"YLLS":paymentOrderId,"ZFDH":PayNo,"SJCQM":SignMsg,"JYJE":JYJE};
	url="/UnionpayCommunications/insertW_XSDD_TL_YLLS.action?json="+JSON.stringify(jsonData);
	visitService(url);
	//window.location="/jlo2o_bd/customer/sydjt/index.html";
