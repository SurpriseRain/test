...
<%
	...
	if (ValidSign.equals(newsign) ) {
		...
		if ("SUCCESS".equals(return_code)) {
			...
			//即时到账处理业务开始
			//------------------------------
		 	String urlvalue=JlAppResources.getProperty("WEIXIN_HEADER")+"/WeixinController/update_notify_url.action?out_trade_no="+out_trade_no+"&trade_no="+transaction_id+"&status=1";
	 	 ...
	 	 String urlvalue=JlAppResources.getProperty("WEIXIN_HEADER")+"/WeixinController/update_notify_url.action?out_trade_no="+out_trade_no+"&trade_no="+transaction_id+"&status=0";
		 URL url = new URL(urlvalue);
         HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
