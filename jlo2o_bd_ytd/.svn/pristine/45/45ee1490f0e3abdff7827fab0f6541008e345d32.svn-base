...
uploadFile.write = function(o,cID,initField,hidefield,record,disabled){
	...
	$.each(uploadFile.bdxx,function(i,bdxx){
		...
		var zdid = bdxx.zdid;
			//附件上传
				...
				$(eles).attr("filenum",bdxx.filenum);//text
				...
				if($(eles).attr("file") != null){
					...
					$(eles3).attr("id","upload_"+zdid);
					$(eles3).attr("onclick","form_uploadfile(this,"+JSON.stringify(initField)+")");
					$(eles3).attr("value","上传");
				...
				var down ="<span style='border:0px solid #e4e4e4;cursor:pointer; float:left; margin:-10px; height:100px;line-height:100px; '>";
				    down +="<a id='A_file_"+zdid+"'  onclick=\"form_downloadfile('"+fileUrl+"',"+$(eles).attr('file')+")\" style='color: blue'>"+
				                   "<img src=\""+fileUrl+"\" style=\"height:60px\"/></a>";
					if(disabled==false){
						//删除附件
					down +="<img src='/img/delete123.gif' onclick=\"delectFile('"+fileName+"','"+fileUrl+"','"+zdid+"',this)\"/>"
					}
					down +="</span>&nbsp";
...
uploadFile.init = function(o,cID,initField,hidefield,record,disabled) {
    uploadFile.load($(o).attr('url'),record);
    uploadFile.write(o,cID,initField,hidefield,record,disabled);
}
...
//点击上传
function form_uploadfile(obj,initField){
	var fid=$(obj).attr("id");
	var bid=fid.substr(7,fid.length);
	if($("[name="+bid+"]").attr("filenum")==1){
		var fileArray=$("[name="+bid+"]").val()==""?[]:JSON.parse($("[name="+bid+"]").val());
	...
	if($("[name="+bid+"]").attr("file")==2){
		...
		var fileSize=$("#file_"+bid).context.fileSize;
...
//							if(initField.contains((id+".3"))){
						if(initField.contains(id)){
							$("#"+FILEID).parent().append("&nbsp;<span style='border:1px solid #e4e4e4;cursor:pointer; float:left; margin:0px; height:18px;line-height:18px; width:200px;'><a id='A_"+FILEID+"' onclick=\"form_downloadfile('"+resultData[i].FILE_URL+"',"+$("[name="+id+"]").attr("file")+");\" style='color: blue'>"+resultData[i].FILE_DESC+"</a><img src='/img/delete123.gif' onclick=\"delectFile('"+resultData[i].FILE_DESC+"','"+resultData[i].FILE_URL+"','"+id+"',this)\"/></span>");
//							$("#"+FILEID).parent().append("&nbsp;<span style='border:1px solid black;cursor:pointer'>" +
//															"<a id='A_"+FILEID+"' onclick=\"form_downloadfile('"+resultData[i].FILE_URL+"',"+$("[name="+id+"]").attr("file")+");\" style='color: blue'>"+resultData[i].FILE_DESC+"</a>" +
//															"<img src='/img/delete123.gif' onclick=\"delectFile('"+fileName+"','"+fileUrl+"','"+zdid+"',this)\"/>" +
//															"</span>");
						}else{
							$("#"+FILEID).parent().append("<span style='border:1px solid #e4e4e4;cursor:pointer; float:left; margin:0px; height:18px;line-height:18px; width:200px;'><a id='A_"+FILEID+"' onclick=\"form_downloadfile('"+resultData[i].FILE_URL+"',"+$("[name="+id+"]").attr("file")+");\" style='color: blue'>"+resultData[i].FILE_DESC+"</a></span>");
						}
						//}
