	...
	$.fn.kkPages = function(options){
		...
		return this.each(function(){		
			...
			//if($AllMth > $Mth){
				...
				var $excelExport = $this.find(".excelExport"); //导出
				//导航指向
				...
				$excelExport.click(function(){
					var realColumns="";
					$.each($ColumnsArr,function(i,value){
						if(realColumns==""){
							realColumns=value.property+":"+value.header;
						}else{
							realColumns=realColumns+";"+value.property+":"+value.header;
						}
					});
					var url;
					if(PagesMth==1){
						...
						$.each(DefStaPer.data,  function(i, val) {
							var minData="";
							 $.each(DefStaPer.columnsArr,function(index,temp){
								 var property=temp.property;
								 var header=temp.header;
								 var fieldValue = eval("val."+property);
								 if(minData==""){
									 minData=minData+"{\""+property+"\":\""+fieldValue+"\"";
								 }else{
									 if(header!="操作"){
										 minData=minData+","+"\""+property+"\":\""+fieldValue+"\"";
									 }
			...
			function showPages(page){
				...
				for(var i=1;i<=AllMth;i++){
					...
					if(parseInt(page) <= parseInt($NavMth)){
